...
type DirectiveResult = Result<(Option<(u8, u8)>, Vec<(String, String)>), ScanError>;
...
pub enum Event {
    ...
    /// - `bool` is `true` when the source contained an explicit `---` marker.
    /// - `Option<(u8, u8)>` is the `%YAML major.minor` directive, if present.
    /// - `Vec<(String, String)>` contains `%TAG handle prefix` pairs.
    DocumentStart(bool, Option<(u8, u8)>, Vec<(String, String)>),
...
pub struct Parser<T> {
    ...
    keep_tags: bool,
    /// Comments collected since the last drain.
    pub pending_comments: Vec<(Marker, String)>,
}
...
impl<T: Iterator<Item = char>> Parser<T> {
    ...
    /// Drain and return all comments collected since the last call.
    pub fn drain_comments(&mut self) -> Vec<(Marker, String)> {
    ...
    fn scan_next_token(&mut self) -> Result<Token, ScanError> {
        loop {
            ...
            match token {
                None => {
                    return match self.scanner.get_error() {
                        None => Err(ScanError::new(self.scanner.mark(), "unexpected eof")),
                        Some(e) => Err(e),
                    };
                }
                Some(tok) => {
                    if let TokenType::Comment(ref text) = tok.1 {
                        self.pending_comments.push((tok.0, text.clone()));
                    } else {
                        return Ok(tok);
                    }
                }
    ...
    fn document_start(&mut self, implicit: bool) -> ParseResult {
        ...
        match *self.peek_token()? {
            ...
            Token(mark, _) if implicit => {
                ...
                Ok((Event::DocumentStart(false, None, vec![]), mark))
            }
            _ => {
                // explicit document
                self.explicit_document_start()
    ...
    fn parser_process_directives(&mut self) -> DirectiveResult {
        let mut version: Option<(u8, u8)> = None;
        let mut tag_directives: Vec<(String, String)> = Vec::new();
        ...
        loop {
            match self.peek_token()? {
                Token(mark, TokenType::VersionDirective(major, minor)) => {
                    ...
                    // YAML version components are single digits; truncation impossible in practice.
                    #[allow(clippy::cast_possible_truncation)]
                    {
                        version = Some((*major as u8, *minor as u8));
                    }
                }
                Token(mark, TokenType::TagDirective(handle, prefix)) => {
                    // The scanner emits an empty TagDirective("", "") as a sentinel for
                    // unknown/reserved directives (e.g. `%FOO …`). Silently skip them.
                    if handle.is_empty() && prefix.is_empty() {
                        // fall through to skip()
                    } else if tags.contains_key(handle) {
                        return Err(ScanError::new(
                            *mark,
                            "the TAG directive must only be given at most once per handle in the same document",
                        ));
                    } else {
                        tag_directives.push((handle.clone(), prefix.clone()));
    ...
    fn explicit_document_start(&mut self) -> ParseResult {
        let (version, tag_directives) = self.parser_process_directives()?;
        match *self.peek_token()? {
            Token(mark, TokenType::DocumentStart) => {
                self.push_state(State::DocumentEnd);
                self.state = State::DocumentContent;
                self.skip();
                Ok((Event::DocumentStart(true, version, tag_directives), mark))
            }
