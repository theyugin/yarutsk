...
class YamlMapping(_NodeBase):
    ...
    ``__contains__`` / ``__len__`` / ``__iter__`` plus ``keys`` / ``values`` /
    ``items`` / ``get`` / ``update`` / ``pop`` / ``popitem`` / ``setdefault``
    / ``clear`` / ``copy`` / ``__or__`` / ``__ior__``, and:
    ...
    # ── Dict protocol (declared explicitly since `YamlMapping` does not extend `dict`) ──
    ...
    def __or__(self, other: _Mapping[str, Any] | YamlMapping) -> dict[str, Any]: ...
    def __ior__(self, other: _Mapping[str, Any] | YamlMapping) -> YamlMapping: ...
    ...
    @overload
    def pop(self, key: str, default: _D) -> Any | _D: ...
    def popitem(self) -> tuple[str, Any]: ...
    def update(
        self, other: _Mapping[str, Any] | YamlMapping | Iterable[tuple[str, Any]]
    ) -> None: ...
    @overload
...
class YamlSequence(_NodeBase):
    ...
    Implements ``__getitem__`` / ``__setitem__`` / ``__delitem__`` /
    ``__contains__`` / ``__len__`` / ``__iter__`` plus ``append`` / ``extend``
    / ``insert`` / ``pop`` / ``remove`` / ``clear`` / ``copy`` / ``count`` /
    ``index`` / ``reverse`` / ``sort`` / ``__add__`` / ``__iadd__`` /
    ``__mul__`` / ``__rmul__`` / comparison operators, and:
    ...
    # ── List protocol (declared explicitly since `YamlSequence` does not extend `list`) ──
    @overload
    ...
    def __iadd__(self, other: Iterable[Any]) -> YamlSequence: ...
    ...
    def copy(self) -> YamlSequence: ...
    def count(self, value: Any) -> int: ...
    def reverse(self) -> None: ...
    def __reduce__(self) -> tuple[type[YamlSequence], tuple[list[Any]]]: ...
...
class YamlIter(Iterator[YamlNode]):
