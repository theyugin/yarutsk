...
- **Memory per document**: each individual document must fit in memory. Input is read in 8 KiB chunks, while the parsed node graph for a completed document remains resident. For large multi-document streams use `iter_load_all` / `iter_loads_all`; `dump_all` likewise consumes and emits its input incrementally.
- **Deep indentation size**: nesting itself is handled with explicit work stacks, but block-style output grows with both document size and indentation depth. Extremely deep block documents can therefore produce very large output.
