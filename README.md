...
#### Integrating with pydantic, msgspec, and cattrs

The Schema API is enough to plug yarutsk into any of the common Python serialization libraries — each integration is a two-line registration. Two patterns are useful: **tag-based** (preserves the surrounding YAML formatting and comments) and **whole-document** (loses comments because the model layer has no place to carry them).
...
# Conversion
doc.to_python()                        # deep conversion to a plain Python dict
...
# Conversion
doc.to_python()                        # deep conversion to a plain Python list
